import { View, ScrollView, Text, TouchableOpacity, ActivityIndicator, TextInput, Pressable } from "react-native";
import { useState, useEffect } from "react";
import { RotateCw, ChevronDown, ChevronUp, Heart, MessageCircle, Send } from "lucide-react-native";
...
export default function DiscoverScreen() {
  ...
  const loadReviewsForItem = async (menuItemId: number) => {
    try {
      ...
      const response = await apiRequest<Review[]>(`/api/reviews/menu-item/${menuItemId}`);
  ...
  const loadCommentsForReview = async (reviewId: number) => {
    try {
      ...
      const response = await apiRequest<Comment[]>(`/api/reviews/${reviewId}/comments`);
  ...
  try {
    ...
    setReviews(prev => ({
      ...prev,
      [menuItemId]: prev[menuItemId]?.map(review => 
        review.id === reviewId 
          ? { 
              ...review, 
...
const handleAddComment = async (reviewId: number, menuItemId: number) => {
  ...
  try {
    ...
    const response = await apiRequest<any>(`/api/reviews/${reviewId}/comments`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
        ...
        <View className="border-t border-gray-200 p-3">
          {loadingReviews[item.id] ? (
            <View className="items-center py-4">
              ...
              <AppText size="small" className="text-gray-600 mt-2">Loading reviews...</AppText>
            </View>
          ) : reviews[item.id]?.length > 0 ? (
            <View className="space-y-3">
              <AppText bold className="text-gray-900 mb-2">Reviews</AppText>
              {reviews[item.id].map((review) => (
                <View key={review.id} className="border border-gray-200 rounded-lg p-3">
                      ...
                      {loadingComments[review.id] ? (
                        ...
                        <>
                          ...
                          {comments[review.id]?.map((comment) => (
                            <View key={comment.id} className="mb-3 pb-2 border-b border-gray-100">
                              <AppText bold size="small" className="text-gray-900">
                                {comment.user_name || "User"}
                              </AppText>
                              <AppText size="small" className="text-gray-700 mt-1">
                                {comment.comment}
                              </AppText>
                              <View className="flex-row justify-between items-center mt-1">
                                ...
                                >
                                  ...
                                  />
                                  <AppText size="small" className="text-gray-500 ml-1">
                          ...
                          {isAuthenticated && (
                            <View className="flex-row items-center mt-2">
                              <TextInput
                                ...
                                onChangeText={(text) => setNewComments(prev => ({ ...prev, [review.id]: text }))}
                                onSubmitEditing={() => handleAddComment(review.id)}
                              />
                              <TouchableOpacity 
            ...
            <AppText className="text-gray-500 text-center">No reviews yet</AppText>
          )}
        </View>
      )}
    </View>
